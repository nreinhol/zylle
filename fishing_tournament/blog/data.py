...
def get_user_scores_rotauge(request, year=data_utils.COMPETITION_YEAR):
    user_ids = data_utils.get_all_user_ids(request)
    sorted_user_scores = sorted([data_utils.UserScoresRotauge(request, user_id, year) for user_id in user_ids], reverse=True)
    data_utils.set_positions(sorted_user_scores)
    return sorted_user_scores
